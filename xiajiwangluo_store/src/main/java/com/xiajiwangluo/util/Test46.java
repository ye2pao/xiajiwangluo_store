...
public class Test46 {
	...
	public void ExportNoResponse(String sheetName, String titleName,
								 ...
								 String[] columnName,String[][] dataList,String name) throws Exception {
		if (columnNumber == columnWidth.length&& columnWidth.length == columnName.length) {
			...
			for (int i = 0; i < dataList.length; i++) {
				row = sheet.createRow((int) i + 2);//从第几行开始编写数据
			...
			// 第六步，将文件存到指定位置
			try {
				FileOutputStream fout = new FileOutputStream("C:\\Users\\Administrator\\Desktop\\"+name+".xls");
				wb.write(fout);
